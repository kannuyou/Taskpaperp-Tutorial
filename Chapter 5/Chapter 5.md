...
有相当数量的任务，写出来不过几个字的差异，比如我列入 `取景:`  项目中的任务就大同小异，除了具体地点之外没有什么不同。
...
我发现一个更高效的方法，即**把 TaskPaper 作为整个工作的中枢，将待处理的文件、待阅读的文章用 GTD 形式搞定。**
...
如果需要切换任务文件，可以用快捷键 `⌘Command+⇧Shift+\`（`↩Return` 键上方）打开 Tab 视图，上下滑动来切换。
...
自动补全是为了更方便地输入，其实在需要反复输入同一个标签组（但备注可能不同）时，有更简便的方法。典型情况是为一个任务添加多个负责人，就不必一一使用  `@Team(陈)`、`@Team(洪)` ，**直接为一个标签添加多个备注**[^2]，像 `@Team(陈,洪)` 这样就好。注意用**半角逗号** `,` 把各个备注隔开，就可以将它们塞进一个标签里，避免反复使用 `@Team()`。
...
把这个【】脚本放进去，然后打开一个 TaskPaper 任务列表，就可以转换标签了。
