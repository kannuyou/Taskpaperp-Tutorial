...
【改：简单介绍整个项目需要做什么，分成哪几个板块】

每天早晨我都用 TaskPaper 来回顾前日、安排当日，把它当做展板（公告栏），时时注意各项任务的进度。
...
【改：引入不够易懂】有相当数量的任务，写出来不过几个字的差异，比如我列入 `取景:`  项目中的任务就大同小异，除了具体地点之外没有什么不同。

此时就犯不着每次重新输入，只要创建第一个任务，将其作为模板进行「克隆」。使用快捷键 `⌘Command+⇧Shift+D` 能够把当前任务克隆出来；如果选中多个任务再使用快捷键，则可以将它们整组克隆。【改：和复制粘贴的区别是什么？】

【克隆任务】

这里再讲讲选中任务的键盘操作。的确，你可以用光标选中任务，但是快捷键提供了更精确、干脆的操作。想选中当前任务，就按 `⌘Command+→` 把光标移到任务末尾，再使用 `⌘Command+⇧Shift+←` 选中那一整行任务。如果想批量选择任务，可以把光标移到行首，按住 `⇧Shift` 的同时，再每按一次 `↓` 或 `↑` 就往下或往上选中一条任务。
...
你一定熟知了「两分钟原则」，的确，干掉一件小任务并不难。困难的是那些棘手的任务——对于我来说，最费时费力的就是取景。大清早就拔除这枚眼中钉，之后一整天都会轻松许多。

取景任务则被我标为最高优先级 `@P1`，并通过 StyleSheet 把标签渲染为显眼的红色。如果你更偏爱精力标签，也可以用 `@focus`。早起干掉取景任务获得的一个惊喜是，我捕捉到了往期制作者没有注意到的晨读、日出、湖光等镜头。
...
一个时间跨度较大的项目，常常包含了不少需要循环执行的任务，例如每天给相机充足电、每周二和周末开总结会议，我不用反复创建它们。使用 TaskPaper 的标签组，标注出专门的循环任务。

【改：Taskpaper 不能设置循环任务，所以采用这种折中的方法】

就拿充电来说，可以这样记录它：`- 单反充电 @today(ev)`，表示每一个「今天」——也就是天天——都要充电。类似的，每周五就是 `@Fri(ev)`。
...
我发现一个更高效的方法，即**把 TaskPaper 作为整个工作的中枢，将待处理的文件、待阅读的文章用 GTD 形式搞定。**【改：突然从一个很大的描述跳到一个很细的需求】
...
著名任务管理工具 [Things 3](https://culturedcode.com/things/whats-new/) 就采用了多窗口来显示不同类型的任务，TaskPaper 里的多窗口其实就是打开多个任务文档，我们可以通过快捷键和文件路径让它用起来更高效。
...
如果需要切换任务文件，可以用快捷键 `⌘Command+⇧Shift+\`（`↩Return` 键上方）打开 Tab 视图，上下滑动来切换。【改：也可以用 `Control+Tab` 和 `Control+Shift+Tab`】
...
出于节省屏幕空间的目的，我把几个大小一样的 TaskPaper 窗口叠放在一起，这样就感觉不到自己实际上在用多个文件。至于为什么不用 Tab  栏，因为它也占用了一行空间啊……【改：也可以 `Command-～` 来切换】
...
自动补全是为了更方便地输入，其实在需要反复输入同一个标签组（但备注可能不同）时，有更简便的方法。典型情况是为一个任务添加多个负责人，就不必一一使用  `@Team(陈)`、`@Team(洪)` ，**直接为一个标签添加多个备注**[^2]，像 `@Team(陈,洪)` 这样就好。注意用**半角逗号** `,` 把各个备注隔开，就可以将它们塞进一个标签里，避免反复使用 `@Team()`。【改：没看懂】
...
把这个【】脚本放进去，然后打开一个 TaskPaper 任务列表，就可以转换标签了。【改：怎么运行脚本？】
