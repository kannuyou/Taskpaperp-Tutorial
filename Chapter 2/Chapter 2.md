...
还有一种任务，不需要如此循规蹈矩。比如说 `- 夜跑 5km`，时间相对固定、内容也简单，我就用简单的中文记录一下，再用英文字母进行搜索时这些重要程度不高的任务就不会显示在最前面。也有人喜欢把这类「每日琐事」单独存入笔记应用 ，这也是一个可行的方法，不过我认为用 TaskPaper 统一管理所有任务更值得一试。
...
第二种是使用 `@start` 或 `@due`，或者把它们结合起来，表示任务的时间跨度。比如 `- 写检察院实习报告 @start(2017-09-25) @due(2017-10-08)` 这样一条任务，它的跨度就是九月底到十月初，如果今天恰好在这个区间内，那么这个任务就属于「今天」了。想找到这些任务很简单，我们直接使用 sidebar 里的提供的「Today」，点一下就会呈现今日任务。它还把带有 `@today` 标签的任务也归进来了，你不会漏掉方才手动打标签的任务。
