...
选择动词的时候，其实已经参杂了我对这个任务的耗时预期，影响了之后的时间分配。比如《拿摄影机的人》这部作品，由于它年代久远，不一定能够一搜就搜到，`Search` 动作本身就意味着我打算花十几二十分钟去找它。但如果我想找的是《尤利西斯》这样人尽皆知的书籍，只要记录一个 `- Download：《尤利西斯》` 就行，我知道 Kindle 商店里肯定有这本书，下载它只消两三分钟。一个任务可能。【改：前面这句话没写完？】我想设计师朋友找素材、文职工作人员找文献时，一定都遇到过超出预期时间的情况，**不如根据对任务的预期，做一个大概的用时估计，先做容易做的，再处理那些不确定的。**

【【【导图】

还有一种任务，不需要如此循规蹈矩。比如说 `- 夜跑 5km`，时间相对固定、内容也简单，我就简单的中文记录一下，用英文字母进行搜索的时候，这些没必要一直在眼前晃的任务，就不会显示出来。【改：为什么不记日历？】
...
第二种是使用 `@start` 或 `@due`，或者把它们结合起来，表示任务的时间跨度。比如 `- 写检察院实习报告 @start(2017-09-25) @due(2017-10-08)` 这样一条任务，它的跨度就是九月底到十月初，如果今天恰好在这个区间内，那么这个任务就属于「今天」了。你一定想问，那怎么过滤出这个任务呢？【改：这句话问得有点突兀，前面刚说了「Today」可以过滤出这个任务，这里又突然问怎么过滤。】本章暂不介绍复杂的过滤语法，我们直接使用 sidebar 里的提供的「Today」，点一下即过滤完毕。它还把带有 `@today` 标签的任务也归进来了，你不会漏掉方才手动打标签的任务。
...
除了给自己看的任务，我也做了一个特殊的共享列表【改：修改措辞，「共享」听起来像同步。】。其实上文提到的 `- 归还：数据线 @Teresa @due(today 11:00)` （完整版）任务我就是和 Teresa 小姐共享的，算是提醒她一下，当天中午我要归还数据线，如果可以的话请尽量待在学校。等等，把这么一串很 Geek 的字符发过去，怕不是要被当成自以为是的傻瓜？其实 TaskPaper 可以直接导出任务到系统提醒事项，我得以把一些涉及其他成员的任务共享出去（篇幅原因将附在最后几章讲）。
